...
### Added
- **What is still waiting on you** (`bin/iak-pending.mjs`): asks every daemon the fleet knows about - this machine's plus every `watchdog-roster.json` entry with a `gate` - for its pending intents and prints them oldest first with the age in the leading column. Motivation: confirmation cards post to the room correctly and then scroll away; on 2026-09-20 four approval requests sat pending on one machine for up to 9 hours 12 minutes and the owner asked twice in one night what was waiting on him, because no surface anywhere could answer it. A host that could not be reached is reported under its own `COULD NOT ASK` heading and is never folded into "nothing pending"; a gate naming another machine by LAN IP is refused rather than probed and reported the same way. A daemon whose rows carry no `status` this version understands is a shape mismatch rather than an empty queue, and a fleet roster that exists but cannot be parsed or read exits `2` instead of silently shrinking the fleet to this machine (only a *missing* roster means "single-machine install"). An age that cannot be believed - missing, future-dated, or epoch-seconds - renders as `age unknown`, sorts to the top and survives `--older-than`, rather than being clamped to "fresh". Exit codes: `0` all clear, `10` pending, `11` some hosts unreachable, `12` no host reachable, `2` cannot run, precedence `12 > 11 > 10 > 0`; `--older-than` narrows what is printed and announced but can never produce a `0`. `--json` for machines. `--room` requires `--older-than` and announces each item at most once per escalation (threshold, then each doubling of its age), keeping a ledger at `~/.iak/pending-announced.json` that is written atomically before the post, carries forward entries for hosts it could not ask, rolls back when a post fails, and degrades to silence - never a storm - when it is missing, corrupt or unwritable. `examples/pending-launchd.plist` wires it to a timer. The gate token is resolved by the existing `resolveGateToken` helper, is sent only to hosts that helper already trusts, and never appears in output or argv.
...
### Changed
- **Node.js floor is 20, stated once.** `package.json` engines was `>=18`, the README said 18, and the installer already required 20. The test suite uses the option-object form of `mock.timers.enable` (Node 20.11+/21.2+), and Node 18 reached end-of-life on 2025-04-30, so engines is now `>=20` and the README matches the installer.
- **New-user onboarding in the README.** A Quick Start (agent key, room, install, the config fields, `rooms watch`, first message) now comes first, followed by Install; the v0.10.x release notes moved below the setup path. The `npm install -g ide-agent-kit` line is marked stale (npm carries 0.2.0) in favour of the installer or a git clone. Examples and example configs use neutral names (`my-room`, `@myagent`, `YOUR_AGENT_KEY`) instead of the maintainers' own rooms and handles, the License section links this repository instead of the old `team-relay` name, and three dead in-page anchors are fixed. Env-var tables now say plainly that the legacy `room-poll-check.py` fallbacks are the maintainers' rooms and handles; the fallbacks themselves are unchanged so existing deployments keep working.
